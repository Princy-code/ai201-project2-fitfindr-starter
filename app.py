...
def handle_query(user_query: str, wardrobe_choice: str) -> tuple[str, str, str]:
    """
    Called by Gradio when the user submits a query. Calls run_agent() and maps
    the session dict to the three output panels.
    ...
    # 4. Error path: show the agent's message in the first panel, clear the rest.
    if session["error"]:
        return f"\u26a0\ufe0f {session['error']}", "", ""

    # 5. Happy path: format the selected listing and return all three outputs.
